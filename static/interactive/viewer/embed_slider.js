...
function sceneRelativeUrl(path) {
  if (!path) {
    return "";
  }
...
function configureInputPanel() {
  if (!showInputPanel || !inputScreen || !inputImage) {
    return;
  }
  const imagePath = config?.input_image || config?.thumbnail || `thumbs/${episodeName}.png`;
  inputImage.src = sceneRelativeUrl(imagePath);
  inputImage.alt = `Input image for scene ${episodeName}`;
  inputScreen.hidden = false;
  document.body.classList.add("has-input-panel");
  sizeCanvas();
}

function clampStep(stepIndex) {
  const maxStep = Math.max(0, (config?.steps?.length || 1) - 1);
...
function configureRevealControl(initialStep) {
  if (!revealSlider || !config?.steps?.length) {
    return;
  }
  revealSlider.min = "0";
  revealSlider.max = String(config.steps.length - 1);
  revealSlider.step = "1";
  syncRevealControl(initialStep);
}
...
async function loadStep(stepIndex, { force = false } = {}) {
  ...
  const clampedStep = clampStep(stepIndex);
  if (!force && clampedStep === activeStep) {
    return;
  }
  activeStep = clampedStep;
  syncRevealControl(clampedStep);
...
const initialStep = clampStep(
  Number.isFinite(requestedInitialStep)
    ? requestedInitialStep
    : 0,
);
configureRevealControl(initialStep);

loadStep(initialStep, { force: true }).catch((error) => {
  console.error(error);
